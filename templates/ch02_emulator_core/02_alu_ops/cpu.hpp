#pragma once
// LAB-8 CPU core — Chapter 2, exercise 02: ALU operations and flag semantics.
//
// The fetch/decode/execute loop from exercise 01 is given complete. Your
// work is the arithmetic itself: wrapping ADD/SUB plus the Z and C flags,
// exactly as specified in SPEC.md. Flags are machine-visible state: LOAD,
// STORE, LOADM, JMP and JZ never touch them, so a flag set by one ALU op
// survives until the next ALU op.
#include <cstddef>
#include <cstdint>
#include <span>
...
// --- Given: fetch / decode / step / run (exercise 01 solutions) ------------
...
//@LABS-BEGIN 1
//@LABS-SOLUTION
// ADD r[a], r[b]: wrap to 8 bits, then set flags from the WIDE result.
// Computing flags before truncation is the classic ALU bug source — the
// carry lives above bit 7 and disappears if you truncate first.
...
//@LABS-STUB
inline void execute_add(Cpu& c, const Decoded& d, StepResult& res) {
    // TODO(1): ADD r[d.a], r[d.b] with wrap-to-8-bit semantics.
    // Set c.zero when the (truncated) result is 0, c.carry when the wide
    // sum exceeded 0xFF. Costs 4 cycles. Flags must be computed from the
    // pre-truncation sum, not the wrapped byte.
    res.cycles = 4;  // stub makes progress so a red test cannot hang run()
    (void)c;
    (void)d;
    (void)res;
}
...
//@LABS-BEGIN 2
//@LABS-SOLUTION
// SUB r[a], r[b]: wrap to 8 bits; C is the BORROW flag (set when the true
// difference went negative, i.e. r[a] < r[b] unsigned). Borrow-before-truncate
// mirrors the carry rule of ADD.
...
//@LABS-STUB
inline void execute_sub(Cpu& c, const Decoded& d, StepResult& res) {
    // TODO(2): SUB r[d.a], r[d.b] with wrap-to-8-bit semantics.
    // c.zero on a zero result; c.carry is the BORROW flag: set exactly when
    // r[d.a] < r[d.b] unsigned BEFORE the subtraction. Costs 4 cycles.
    res.cycles = 4;  // stub makes progress so a red test cannot hang run()
    (void)c;
    (void)d;
    (void)res;
...
// Dispatch covers the ops introduced so far; later exercises extend it.
inline void execute(Cpu& c, const Decoded& d, StepResult& res) {
    switch (d.op) {
        case Op::Halt:
            c.halted = true;
