...
//@LABS-STUB
    // TODO({n}): implement FX29 (I = address of glyph for low nibble of VX).
    bool op_font(uint16_t op) {
        (void)op;
...
//@LABS-STUB
    // TODO({n}): implement FX55 store V0..VX honoring load_store_leaves_i
    // and vf_reset quirks.
    bool op_store_v(uint16_t op) {
        (void)op;
        return false;  // stub halts the machine; suite runs RED until done
...
//@LABS-STUB
    // TODO({n}): implement FX65 load V0..VX honoring load_store_leaves_i
    // and vf_reset quirks.
    bool op_load_v(uint16_t op) {
        (void)op;
        return false;  // stub halts the machine; suite runs RED until done
...
//@LABS-END
    ...
    bool op_mem(uint16_t op) {
        switch (op & 0xFFu) {
            ...
            default:
                return false;  // timers/input-wait belong to ch05
        }
    }
    ...
    bool exec(uint16_t op) {
        switch (op >> 12) {
            ...
            default:  return false;  // CXNN/EX** arrive next exercise
        }
    }
};
