...
//@LABS-STUB
    // TODO({n}): implement seed_rng() and one deterministic LCG step per
    // next_random() call.
    void seed_rng(uint64_t s) { (void)s; /* TODO */ }
    uint8_t next_random() { return 0; /* TODO */ }
//@LABS-END
//@LABS-BEGIN 2
//@LABS-SOLUTION
...
//@LABS-STUB
    // TODO({n}): implement CXNN (VX = random byte masked by NN).
    bool op_rand(uint16_t op) {
        (void)op;
...
//@LABS-STUB
    // TODO({n}): implement EX9E (skip if key VX pressed) and EXA1
    // (skip if not pressed).
    bool op_key(uint16_t op) {
        (void)op;
        return false;  // stub halts the machine; suite runs RED until done
