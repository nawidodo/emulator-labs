...
//@LABS-STUB
    // TODO({n}): implement 8XY4 add with VF = carry out of bit 7.
    bool op_add_carry(uint16_t op) {
        (void)op;
...
//@LABS-STUB
    // TODO({n}): implement 8XY5 and 8XY7 subtract with VF = NOT borrow.
    bool op_sub_borrow(uint16_t op) {
        (void)op;
        return false;  // stub halts the machine; suite runs RED until done
    }
...
//@LABS-STUB
    // TODO(3): implement 8XY6/8XYE honoring quirks.shift_uses_vy,
...
//@LABS-STUB
    // TODO({n}): implement FX33 BCD store (I[0]=VX/100, then tens, ones).
    bool op_bcd(uint16_t op) {
        (void)op;
        return false;  // stub halts the machine; suite runs RED until done
    }
...
//@LABS-STUB
    // TODO(5): implement FX55/FX65 honoring load_store_leaves_i and
    // vf_reset quirks.
    bool op_store_v(uint16_t op) {
        (void)op;
        return false;
