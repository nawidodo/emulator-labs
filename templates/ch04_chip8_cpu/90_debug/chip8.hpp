...
//@LABS-BEGIN 1
//@LABS-SOLUTION
    // 8XY6/8XYE: quirk-selected source; VF = bit shifted out, captured
    // BEFORE the shift.
...
//@LABS-STUB
    // Plausible-looking shift... but VF always reads VY.
    // TODO(1): this family carries one SEEDED DEFECT - find it, fix it,
    // and document it per DEBUGGING.md before moving on.
    bool op_shift(uint16_t op) {
        const uint8_t x = uint8_t((op >> 8) & 0xF);
        const uint8_t y = uint8_t((op >> 4) & 0xF);
        const uint8_t src = quirks.shift_uses_vy ? v[y] : v[x];
        if ((op & 0xFu) == 0x6) {
            v[0xF] = v[y] & 0x1u;      // BUG: flag source ignores the selection
            v[x] = uint8_t(src >> 1);
        } else {
            v[0xF] = (v[y] >> 7) & 0x1u;
            v[x] = uint8_t(src << 1);
...
//@LABS-BEGIN 2
//@LABS-SOLUTION
    // 8XY5 / 8XY7 subtract forms with VF = NOT borrow.
...
//@LABS-STUB
    // Plausible-looking subtract... but 8XY7's borrow direction is inverted:
    // it copies 8XY5's comparison instead of mirroring it.
    // TODO(2): this family carries one SEEDED DEFECT - find it, fix it,
    // and document it per DEBUGGING.md before moving on.
    bool op_sub_borrow(uint16_t op) {
        const uint8_t x = uint8_t((op >> 8) & 0xF);
        const uint8_t y = uint8_t((op >> 4) & 0xF);
        if ((op & 0xFu) == 0x5) {
            ...
            v[0xF] = v[x] >= v[y] ? 1 : 0;  // BUG: backwards comparison
            v[x] = uint8_t(v[y] - v[x]);
        }
        return true;
    }
...
//@LABS-STUB
    // Plausible-looking conditional skip... but 4XNN behaves like 3XNN.
    // TODO(3): this family carries one SEEDED DEFECT - find it, fix it,
    // and document it per DEBUGGING.md before moving on.
    bool op_skip_imm(uint16_t op) {
        const uint8_t x = uint8_t((op >> 8) & 0xF);
        const bool equal = v[x] == (op & 0xFFu);
        if (equal) pc = wrap12(uint16_t(pc + 2));  // BUG: SNE skips on equality
        return true;
    }
//@LABS-END
//@LABS-BEGIN 4
//@LABS-SOLUTION
    // FX33: BCD digits stored hundreds-first at I, I+1, I+2.
...
//@LABS-STUB
    // Plausible-looking BCD store... but hundreds and ones trade places.
    // TODO(4): this family carries one SEEDED DEFECT - find it, fix it,
    // and document it per DEBUGGING.md before moving on.
    bool op_bcd(uint16_t op) {
        const uint8_t val = v[(op >> 8) & 0xFu];
        mem[wrap12(idx)]     = uint8_t(val % 10);   // BUG: digit order swapped
        mem[wrap12(idx + 1)] = uint8_t((val / 10) % 10);
        mem[wrap12(idx + 2)] = uint8_t(val / 100);  // BUG: digit order swapped
        return true;
    }
//@LABS-END
    ...
    // Route the 8XY* family; every family member is present here - one of
    // them misbehaves (tests pinpoint the symptom).
    ...
    // Disassembler (curriculum SS55): mnemonic view of memory at addr,
    // independent of PC. Exact enough to diff against trace logs.
    std::string disassemble(uint16_t addr) const {
        char buf[48];
        const uint16_t op =
            uint16_t(uint16_t(mem[addr]) << 8 | mem[wrap12(uint16_t(addr + 1))]);
        const unsigned nnn = op & 0x0FFFu;
        const unsigned x = (op >> 8) & 0xFu;
        const unsigned y = (op >> 4) & 0xFu;
        const unsigned nn = op & 0xFFu;
        switch (op >> 12) {
            case 0x0:
                if (op == 0x00EE) return "00EE  RET";
                break;
            case 0xB:
                std::snprintf(buf, sizeof(buf), "%04X  JP    V0+0x%03X", op, nnn);
                return buf;
            case 0x1: std::snprintf(buf, sizeof(buf), "%04X  JP    0x%03X", op, nnn); return buf;
            case 0x2: std::snprintf(buf, sizeof(buf), "%04X  CALL  0x%03X", op, nnn); return buf;
            case 0x3: std::snprintf(buf, sizeof(buf), "%04X  SE    V%X,0x%02X", op, x, nn); return buf;
            case 0x4: std::snprintf(buf, sizeof(buf), "%04X  SNE   V%X,0x%02X", op, x, nn); return buf;
            case 0x5: std::snprintf(buf, sizeof(buf), "%04X  SE    V%X,V%X", op, x, y); return buf;
            case 0x6: std::snprintf(buf, sizeof(buf), "%04X  LD    V%X,0x%02X", op, x, nn); return buf;
            case 0x7: std::snprintf(buf, sizeof(buf), "%04X  ADD   V%X,0x%02X", op, x, nn); return buf;
            case 0xA: std::snprintf(buf, sizeof(buf), "%04X  LD    I,0x%03X", op, nnn); return buf;
            case 0xC: std::snprintf(buf, sizeof(buf), "%04X  RND   V%X,0x%02X", op, x, nn); return buf;
            default: break;
        }
        switch (op >> 12) {
            case 0x8: {
                static const char* kMn[16] = {"LD","OR","AND","XOR","ADD","SUB",
                    "SHR","SUBN","??","??","??","??","SHL","??","??","??"};
                std::snprintf(buf, sizeof(buf), "%04X  %-4s  V%X,V%X", op,
                              kMn[op & 0xFu], x, y);
                return buf;
            }
            case 0x9: std::snprintf(buf, sizeof(buf), "%04X  SNE   V%X,V%X", op, x, y); return buf;
            case 0xE:
                if (nn == 0x9E) { std::snprintf(buf, sizeof(buf), "%04X  SKP   V%X", op, x); return buf; }
                if (nn == 0xA1) { std::snprintf(buf, sizeof(buf), "%04X  SKNP  V%X", op, x); return buf; }
                break;
            case 0xF:
                switch (nn) {
                    case 0x29: std::snprintf(buf, sizeof(buf), "%04X  LD    F,V%X", op, x); return buf;
                    case 0x33: std::snprintf(buf, sizeof(buf), "%04X  LD    B,V%X", op, x); return buf;
                    case 0x55: std::snprintf(buf, sizeof(buf), "%04X  LD    [I],V%X", op, x); return buf;
                    case 0x65: std::snprintf(buf, sizeof(buf), "%04X  LD    V%X,[I]", op, x); return buf;
                    default: break;
                }
                break;
            default: break;  // 0xD display belongs to ch05
        }
