...
//@LABS-BEGIN 1
//@LABS-STUB
        // TODO(1): fix the bug below.
        // BUG 1: treats a row that merely EXTENDS past an edge as fully
        // invisible, so sprites near the right edge lose entire rows even
        // though their on-screen columns should still render.
        const int abs_y = origin_y + row;
        if (!quirks.wrapping &&
            (abs_y < 0 || abs_y >= kHeight ||
             origin_x + 8 > kWidth || origin_x + 8 < 0)) continue;
...
//@LABS-BEGIN 2
//@LABS-STUB
            // TODO(2): fix the bug below.
            // BUG 2: reports a collision when a sprite bit LIGHTS an empty
            // pixel. Real hardware (and every CHIP-8 game) flags the
            // opposite: a set VF means a lit pixel was ERASED.
...
private:
    ...
    //@LABS-BEGIN 3
    //@LABS-STUB
    // TODO(3): fix the bug below.
    // BUG 3: decrements BOTH timers twice per 60 Hz tick. Symptom: every
    // countdown burns at 120 Hz — delay windows and beeps last half as long
    // as they should.
    void tick_once() {
        for (int k = 0; k < 2; ++k) {
            if (delay > 0) --delay;
...
struct Machine {
    ...
    int step() {
        ...
        switch (op >> 12) {
            ...
            case 0xD:
                if (n == 0) v[0xF] = 0;  // height-0 draw draws nothing
