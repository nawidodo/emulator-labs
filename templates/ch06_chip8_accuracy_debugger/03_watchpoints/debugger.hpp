...
private:
    ...
    bool exec_line(const std::string& line) {
        ...
        if (cmd == "watch") { cmd_watch(is); return false; }
    ...
    void cmd_step(std::istringstream& is) {
        ...
        for (unsigned i = 0; i < n && !halted(); ++i) {
            ...
            cpu_.step();
            std::string hit = watches_.check(cpu_);
            if (!hit.empty()) {
                out_ << hit << "\n";
                return;
            }
    ...
    void cmd_watch(std::istringstream& is) {
        auto spec = ch06::parse_watch_spec(is);
        if (!spec) {
            out_ << "error: bad arguments\n";
            return;
        }
        watches_.add(*spec, cpu_);
        out_ << "watchpoint " << watches_.size() - 1 << " added\n";
    }
    ...
    void cmd_help() {
        out_ << "commands:\n"
             ...
             << "  watch <spec>        break on memory change / register predicate\n"
