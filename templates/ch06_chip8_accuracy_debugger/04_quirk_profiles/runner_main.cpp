...
//
//   ch06_04_runner --rom PATH [--cycles N | --frames N]
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--help" || a == "-h") {
            std::cout << "usage: ch06_04_runner --rom PATH "
    ...
    ch06::Chip8 cpu;
    cpu.reset(ch06::kModernQuirks);
    if (!quirks_spec.empty()) {
        const auto q = ch06::profile_by_name(quirks_spec);
        if (!q) {
