...
//
//   ch06_05_runner --rom PATH [--cycles N | --frames N]
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--help" || a == "-h") {
            std::cout << "usage: ch06_05_runner --rom PATH "
    ...
    auto quirks_opt = ch06::profile_by_name(
        quirks_spec.empty() ? std::string("MODERN") : quirks_spec);
    if (!quirks_opt) {
        std::cerr << "error: unknown quirks profile: " << quirks_spec << "\n";
        return 1;
    }
    const ch06::Chip8Quirks quirks = *quirks_opt;
