#pragma once
// ch06 debugging-exercise core: a CHIP-8 interpreter with ONE seeded bug
// (see DEBUGGING.md). The @LABS block marks the faulty region; everything
// else matches the reference core.
...
// Historical CHIP-8 implementations disagree about a handful of opcodes.
// Every field defaults to the MODERN profile used by most current
// interpreters; presets live in quirks.hpp (exercise 04).
struct Chip8Quirks {
    // FX1E / FX55 / FX65 clear VF before doing their work (modern) or leave
    // VF alone (COSMAC VIP).
    bool vf_reset = true;
    // 8XY6/8XYE shift VY into VX (COSMAC VIP) or shift VX in place (modern).
    bool shift_uses_vy = false;
    // FX55/FX65 leave I untouched (COSMAC VIP) or advance I by X+1 (modern).
    bool load_store_leaves_i = false;
    // DXYN wraps around screen edges (COSMAC VIP) or clips (modern).
    bool wrapping = false;
    // BNNN adds register X encoded in the opcode's high nibble of the low
    // byte (CHIP-48 bug) instead of V0.
    bool jump_bnnn_x = false;
};

inline constexpr Chip8Quirks kModernQuirks{};
inline constexpr Chip8Quirks kCosmacVipQuirks{
    /*vf_reset*/ false, /*shift_uses_vy*/ true,
...
//@LABS-STUB
            case 0x55:
                // TODO(1): this handler carries a SEEDED BUG — find it by
                // tracing a save/reload program against the golden trace,
                // fix it here, then write bug-report.md per DEBUGGING.md.
                if (quirks->vf_reset) v[0xF] = 0;
                for (int r = 0; r <= x; ++r) mem[(i + r) & 0xFFF] = v[r];
                i = uint16_t(i + x + 1);  // <- suspicious: unconditional?
                break;
            case 0x65:
                if (quirks->vf_reset) v[0xF] = 0;
                for (int r = 0; r <= x; ++r) v[r] = mem[(i + r) & 0xFFF];
