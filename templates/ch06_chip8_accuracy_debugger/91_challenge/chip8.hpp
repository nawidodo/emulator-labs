#pragma once
// ch06 CHIP-8 core for exercise 04: identical to the 01_tracing reference
// core except that the four quirk-sensitive execution sites are @LABS
// tasks. Chip8Quirks and the named presets live in quirks.hpp.
...
//@LABS-SOLUTION
                    case 0x6: {  // shift right; VF takes the dropped bit
                        const uint8_t src =
                            quirks->shift_uses_vy ? v[y] : v[x];
                        v[0xF] = src & 0x01;
                        v[x] = uint8_t(src >> 1);
                        break;
                    }
//@LABS-STUB
                    case 0x6: {  // TODO(2): honor quirks->shift_uses_vy
                                 // (VIP/CHIP48 shift VY into VX)
                        v[0xF] = v[x] & 0x01;
                        v[x] = uint8_t(v[x] >> 1);
                        break;
                    }
...
//@LABS-STUB
                    case 0xE: {  // TODO(3): honor quirks->shift_uses_vy
                        v[0xF] = (v[x] >> 7) & 0x01;
                        v[x] = uint8_t(v[x] << 1);
                        break;
                    }
...
//@LABS-SOLUTION
            case 0xB:
                ...
                break;
//@LABS-STUB
            case 0xB:  // TODO(4): honor quirks->jump_bnnn_x (CHIP-48 adds VX)
                pc = uint16_t(nnn + v[0]);
                break;
//@LABS-END
...
//@LABS-STUB
                int px, py;  // TODO(5): wrap instead of clip when
                             // quirks->wrapping is set
                px = ox + colbit;
                py = oy + row;
                if (px >= kScreenW || py >= kScreenH) continue;
//@LABS-END
...
//@LABS-SOLUTION
            case 0x1E:
                ...
                break;
//@LABS-STUB
            case 0x1E:  // TODO(6): clear VF first when quirks->vf_reset
                i = uint16_t(i + v[x]);
                break;
//@LABS-END
...
//@LABS-STUB
            case 0x55:
                // TODO(7): clear VF when quirks->vf_reset, and keep I
                // untouched when quirks->load_store_leaves_i.
                for (int r = 0; r <= x; ++r) mem[(i + r) & 0xFFF] = v[r];
                i = uint16_t(i + x + 1);
                break;
            case 0x65:
                // TODO(7): same two quirks as FX55 above.
                for (int r = 0; r <= x; ++r) v[r] = mem[(i + r) & 0xFFF];
                i = uint16_t(i + x + 1);
                break;
//@LABS-END
