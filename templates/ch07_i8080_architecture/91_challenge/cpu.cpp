...
//@LABS-STUB
    // TODO(1): implement fetch8/fetch16 (little-endian, PC advance) and
    // get_reg/set_reg register-field access (field 6 = memory at HL).
    auto fetch8 = [&]() -> uint8_t {
        (void)start_pc;
        return 0;  // wrong on purpose
    };
    auto fetch16 = [&]() -> uint16_t { return 0; };  // wrong on purpose
    auto get_reg = [&](uint8_t idx) -> uint8_t {
        (void)idx;
        return 0;  // wrong on purpose
    };
    auto set_reg = [&](uint8_t idx, uint8_t v) { (void)idx; (void)v; };
//@LABS-END
...
//@LABS-STUB
    // TODO(2): implement MOV r,r' (0x40-0x7F except 0x76=HLT, 5T/7T for M)
    // and MVI r,data (00DDD110, 7T/10T for M).
    if ((opcode & 0xC0) == 0x40 && opcode != 0x76) {
        (void)opcode;
        cost = 4;  // wrong on purpose (nonzero so run() loops terminate)
        cycles += cost;
        return cost;
    }
//@LABS-END
...
//@LABS-STUB
    // TODO(3): implement LXI (00RP0001, pairs BC/DE/HL, 10T), LDA addr
    // (0x3A, 13T) and STA addr (0x32, 13T). Note SP-form LXI arrives in
    // chapter 8's stack work; skip (opcode & 0x30) == 0x30 here.
    if (false) {
        cost = 4;  // wrong on purpose (nonzero so run() loops terminate)
        cycles += cost;
        return cost;
...
//@LABS-STUB
    // TODO(4): implement INR (00DDD100) and DCR (00DDD101): update value,
    // AC via aux_carry_add/sub, S/Z/P of result, CY preserved, 5T/10T(M).
    if ((opcode & 0xC7) == 0x04 || (opcode & 0xC7) == 0x05) {
        cost = 4;  // wrong on purpose (nonzero so run() loops terminate)
        cycles += cost;
        return cost;
...
//@LABS-STUB
    // TODO(5): dispatch the ALU group (ADD/ADC/SUB/SBB/ANA/XRA/ORA/CMP,
    // register 10OOOSSS + immediate 11OOO110). Commit CY/AC/S/Z/P; write
    // back A only for non-CMP groups. Timing: 4T reg, 7T memory or imm.
...
//@LABS-STUB
    // TODO(6): handle NOP (0x00, 4T) and HLT (0x76, sets halted, 7T).
    if (opcode == 0x00 || opcode == 0x76) {
        cost = 4;  // wrong on purpose (nonzero so run() loops terminate)
        cycles += cost;
        return cost;
    }
