...
//@LABS-STUB
    // TODO(1): implement JMP/Jcc. Conditional jumps cost 10T when taken,
    // 7T when not. Condition codes: NZ Z NC C PO PE P M (bits 5-3).
    if (false) {
...
//@LABS-STUB
    // TODO(2): implement CALL/Ccc: push PC as return address, jump to the
    // target. Taken calls take 17T; a NOT-taken conditional call still
    // burns 11T (it fetched the address anyway).
    if (false) {
        cost = 0;  // wrong on purpose
        cycles += cost;
        return cost;
...
//@LABS-STUB
    // TODO(3): implement RET/Rcc: pop the return address into PC.
    // Unconditional RET is 10T; conditional returns are 11T taken and a
    // mere 5T when skipped (no operand fetch happened).
    if (false) {
        cost = 0;  // wrong on purpose
        cycles += cost;
        return cost;
...
//@LABS-STUB
    // TODO(4): implement PUSH (11RP0101, 11T) and POP (11RP0001, 10T) for
    // BC/DE/HL/PSW. PSW pushes A in the HIGH byte and packed flags in the
    // low byte; POP PSW restores every flag including CY and AC.
    if (false) {
        cost = 0;  // wrong on purpose
        cycles += cost;
        return cost;
...
//@LABS-STUB
    // TODO(5): implement RST n (11NNN111): push PC, jump to n*8, 11T.
    if (false) {
        cost = 0;  // wrong on purpose
        cycles += cost;
        return cost;
...
//@LABS-STUB
    // TODO(6): implement PCHL (E9, jump via HL), SPHL (F9), XCHG (EB),
    // EI (FB) and DI (F3).
    if (false) {
        cost = 0;  // wrong on purpose
        cycles += cost;
...
//@LABS-SOLUTION
    ...
    return true;
//@LABS-STUB
    // TODO(7): accept interrupts only when IFF is set; on acceptance push
    // PC, set PC to (opcode & 0x38), clear IFF, wake from HLT, return true.
    (void)opcode;
