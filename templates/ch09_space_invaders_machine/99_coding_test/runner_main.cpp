...
//
//   si_b_runner --rom prog.bin [--cycles N | --frames N]
...
int main(int argc, char** argv) {
    if (argc > 1 && std::strcmp(argv[1], "--help") == 0) {
        std::cout <<
            "usage: si_b_runner --rom PATH [--cycles N | --frames N]\n"
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--cpf") { opts.cpf = strtoull(next().c_str(), nullptr, 0); opts.has_cpf = true; }
    ...
    sib::MachineB m(sib::kArcade8080B);
...
const uint64_t cpf =
        opts.has_cpf ? opts.cpf
                     : sib::cycles_per_frame(sib::kArcade8080B.khz);
    const uint64_t budget = opts.has_frames
                                ? opts.frames * cpf
                                : (opts.has_cycles ? opts.max_cycles : cpf);
    ...
    while (!m.cpu().halted && m.cpu().cycles < budget) {
        ...
        if (irq.raised) {
            ...
            if (!script.empty()) {
                const size_t idx =
                    size_t(m.cpu().cycles / sib::cycles_per_frame(
                                             sib::kArcade8080B.khz));
                m.set_inputs(
