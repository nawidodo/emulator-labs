...
struct Cpu {
    ...
    // Extension points: later exercises plug whole opcode families in
    // without touching this decoder. Hooks are consulted in order; the first
    // one that returns true wins. (Single-slot in ch10; a chain here because
    // ch11 layers several families.)
    struct Hook {
