...
// timer programs need: ALU, loads, jumps, INC/DEC, PUSH/POP and the hook
// chain. Original authored in ch11 (01_daa_rotates/core.hpp + the stack
// helpers of 02_stack_calls); chapter 13 carries its own copy so every
// exercise tree stands alone.
...
struct Cpu {
    ...
    // Extension points: whole opcode families plug in as hooks without
    // touching this decoder. Hooks are consulted in order; the first one
    // that returns true wins.
    struct Hook {
        bool (*exec)(void* ctx, Cpu& cpu, uint8_t op, int& cycles);
        void* ctx = nullptr;
        Hook* next = nullptr;
    };
    Hook* hooks = nullptr;
    ...
    // Stack helpers (ch11's 02_stack_calls, folded in so timer programs can
    // PUSH around their ISRs): SP-relative, high byte first.
    friend void push16(Cpu& cpu, uint16_t v) {
        cpu.bus->write(cpu.sp - 1, static_cast<uint8_t>(v >> 8));
        cpu.bus->write(cpu.sp - 2, static_cast<uint8_t>(v & 0xFF));
        cpu.sp = static_cast<uint16_t>(cpu.sp - 2);
    }

    friend uint16_t pop16(Cpu& cpu) {
        const uint8_t lo = cpu.bus->read(cpu.sp);
        const uint8_t hi = cpu.bus->read(cpu.sp + 1);
        cpu.sp = static_cast<uint16_t>(cpu.sp + 2);
        return static_cast<uint16_t>(hi << 8 | lo);
    }

    // Decode + execute one opcode (already fetched, PC past it). Returns
    // false for opcodes this trimmed core does not implement (caller traps).
    // Cycle counts come from opcode_meta.hpp; conditional instructions add
    // cycles_alt only when the branch is taken.
    bool exec(uint8_t op, int& cycles) {
        for (Hook* h = hooks; h; h = h->next) {
            if (h->exec(h->ctx, *this, op, cycles)) return true;
        }
        ...
        switch (op) {
            ...
            case 0x76:  // halt (hook-served too; kept for a bare core)
                halted = true;
                return true;
            ...
            // LD (nn),A / LD A,(nn) -- how timer programs reach $FF04-$FF07
            case 0xEA: {
                const uint16_t target = fetch16();
                bus->write(target, a);
                return true;
            ...
            // LDH (n),A / LDH A,(n)
            case 0xE0: bus->write(0xFF00 | fetch8(), a); return true;
            case 0xF0: a = bus->read(0xFF00 | fetch8()); return true;

            // PUSH/POP rp2 (AF pops with F low nibble forced to 0)
            case 0xC5: case 0xD5: case 0xE5: case 0xF5:
                push16(*this, p == 3 ? af() : rp(p));
                return true;
            case 0xC1: case 0xD1: case 0xE1: case 0xF1: {
                const uint16_t v = pop16(*this);
                if (p == 3) { a = static_cast<uint8_t>(v >> 8); f = v & 0xF0; }
                else set_rp(p, v);
                return true;
            }
            ...
            case 0xE6: case 0xEE: case 0xF6: case 0xFE: {
                const int alu_y = (op >> 3) & 7;
                const uint8_t operand = fetch8();
                alu8(alu_y, a, operand, alu_y == 1 || alu_y == 3, alu_y != 7);
                return true;
            }
    ...
    int step() {
        if (trap) return 0;
        if (halted) return 4;  // burn time; wake is the driver's job
