...
inline bool mode_zpx(Cpu& c, uint16_t& out) {
    out = uint8_t(c.fetch8() + c.x);  // zero-page sums wrap in 8 bits
    return false;
}
//@LABS-STUB
// TODO(1): BUG - this version lets the sum escape page zero. $80,X=$90
// must address $0010; find and fix the missing wrap.
inline bool mode_zpx(Cpu& c, uint16_t& out) {
    out = uint16_t(c.fetch8() + c.x);  // BUG: no & 0xFF wrap
    return false;
}
//@LABS-END
...
//@LABS-STUB
// TODO(2): BUG - this (zp),Y stopped reporting page crossings, so reads
// that cross a page run one cycle short. Traces show cyc=5 where the
// reference logs cyc=6 for e.g. LDA ($40),Y with base $20FF, Y=$01.
inline bool mode_izy(Cpu& c, uint16_t& out) {
    const uint8_t ptr = c.fetch8();
    const uint16_t base =
        c.read(ptr) | uint16_t(c.read(uint8_t(ptr + 1))) << 8;
    out = uint16_t(base + c.y);
    return false;  // BUG: crossing information dropped
}
//@LABS-END
