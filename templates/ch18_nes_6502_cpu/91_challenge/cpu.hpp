...
//@LABS-STUB
// TODO(1): JMP ($xxxx) indirect. Watch the page-wrap quirk: a pointer
// ending in $FF reads its high byte from the SAME page ($30FF -> hi from
// $3000), never from the next one.
inline bool mode_ind(Cpu& c, uint16_t& out) {
    const uint16_t ptr = c.fetch16();
    const uint16_t lo = c.read(ptr);
    const uint16_t hi = c.read(uint16_t(ptr + 1));  // naive: crosses pages
    out = lo | hi << 8;
    return false;
}
//@LABS-END
...
//@LABS-STUB
// TODO(2): taken branches bill +1 cycle and +1 more when the target sits
// on a different page than the byte after the operand. This version jumps
// correctly but never pays either penalty - traces run short.
inline void branch_rel(Cpu& c, bool cond) {
    const auto off = int8_t(c.fetch8());
    if (!cond) return;
    c.pc = uint16_t(c.pc + off);  // BUG: no fixup cycles
}
//@LABS-END
...
//@LABS-STUB
// TODO(3): bill the page-cross penalty. OnCross rows pay only when the
// mode reports a crossing; Always rows (indexed stores/RMW) pay every
// time. Without this, abs,X reads and indexed stores log short.
    (void)crossed;  // BUG: penalties never applied
//@LABS-END
    e.op(c, addr);
    return int(c.cycles - t0);
}
