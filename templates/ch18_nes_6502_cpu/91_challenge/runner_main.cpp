// Headless runner for the ch18 challenge: raw 6502 programs on flat RAM.
//
//   ch18_91_challenge_runner --rom prog.bin [--data ADDR=HEX]...
//                           [--cycles N] [--trace FILE]
//
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--rom") {
            ...
            std::sscanf(next(), "%x=%hhx", &addr, &val);
            g_preload.push_back({uint16_t(addr), uint8_t(val)});
    ...
    for (const auto& pre : g_preload) ram.mem[pre.addr] = pre.val;
    if (!program.empty())
