// nes6502.hpp — 6502 core, exercise 02: dummy accesses + unofficials.
//
// This exercise makes the invisible visible: the RMW double-write,
...
// the new one (a dummy write other devices can observe). Billing each
// access gives the official 5-cycle zp / 6-cycle abs RMW totals.
// TODO(1): the RMW helper must write the OLD value back BEFORE the new
// one — a real bus write other devices can observe, and the reason
// INC $zp costs 5 cycles. Sequence: read, write(old), compute, write(new).
using RmwFn = uint8_t (*)(Cpu&, uint8_t);
inline uint8_t rmw(Cpu& c, uint16_t ad, RmwFn f) {
    const uint8_t old = c.read(ad);
    const uint8_t updated = f(c, old);
...
//@LABS-STUB
// TODO(2): unofficial NOPs behave like EA with real timings — implied forms
// burn one internal cycle (total 2); operand-bearing forms must actually
// READ their effective address (that read IS their extra cycle: 3 zp,
// 4 abs/zpx-family, 5 crossed abs,X).
inline void op_nop_u(Cpu&, uint16_t) {}    // TODO(2)
inline void op_nop_mem(Cpu&, uint16_t) {}  // TODO(2)
//@LABS-END
...
//@LABS-STUB
// TODO(3): LAX loads A AND X with memory (N/Z from the value). SAX stores
// A & X and touches NO flags at all.
inline void op_lax(Cpu&, uint16_t) {}  // TODO(3)
inline void op_sax(Cpu&, uint16_t) {}  // TODO(3)
//@LABS-END
...
//@LABS-STUB
// TODO(4): unofficial RMW combos chain a memory modify (through rmw(), so
// the dummy write still happens) into a register operation:
//   DCP = DEC mem, then CMP A     ISB = INC mem, then SBC A
//   SLO = ASL mem, then ORA A     RLA = ROL mem, then AND A
inline void op_dcp(Cpu&, uint16_t) {}  // TODO(4)
inline void op_isb(Cpu&, uint16_t) {}  // TODO(4)
inline void op_slo(Cpu&, uint16_t) {}  // TODO(4)
inline void op_rla(Cpu&, uint16_t) {}  // TODO(4)
//@LABS-END
...
//@LABS-STUB
/// Execute one instruction; returns cycles billed.
inline int step(Cpu& c) {
    if (c.halted || c.bus == nullptr) return 0;
    const uint64_t t0 = c.cycles;
    const Entry& e = kTable[c.fetch8()];
    ...
    switch (e.penalty) {
        case Penalty::OnCross:
            if (crossed) ++c.cycles;  // TODO(5): BUG — cycle billed but the
            break;                    // bus never sees the dummy read
        case Penalty::IndexedAlways:
            ++c.cycles;               // TODO(5): same defect, every time
            break;
        case Penalty::Internal:
            ++c.cycles;
            break;
