// nes6502.hpp — 6502 core, exercise 03: nestest-style trace logging.
//
// This exercise builds the observability layer: an instruction
...
/// mnemonic plus rendered operand — "LDA #$05", "JMP $C5F5", "STA $40,X",
/// "ASL A", bare "RTS". Undecoded rows render as "???".
...
//@LABS-STUB
// TODO(1): render "MNEMONIC [operand]" for the instruction PEEKED at pc.
// Operand shapes: imm "#$XX", zp "$XX", zpx "$XX,X", zpy "$XX,Y",
// abs "$XXXX", ind "($XXXX)", absx "$XXXX,X", absy "$XXXX,Y",
// izx "($XX,X)", izy "($XX),Y", acc "MN A", implied bare mnemonic,
// branches "MN $XXXX" with the ABSOLUTE target, undecoded "???".
// Peek bytes through the bus — never fetch.
inline std::string disassemble_at(Bus&, uint16_t) {
    return std::string("???");  // TODO(1)
}

// TODO(1): instruction length in bytes — implied/acc = 1, imm/zp-family/
// (ind) family = 2, abs family incl ind = 3, undecoded = 1.
inline uint8_t disasm_len(uint8_t) {
    return 1;  // TODO(1)
}

// TODO(1): snapshot pc/op/operand bytes/disassembly into a TraceRow.
inline TraceRow peek_trace(Bus&, uint16_t pc) {
    TraceRow r;
    r.pc = pc;
    return r;  // TODO(1)
}
...
//@LABS-STUB
// TODO(2): format "%04X  " + opcode slot + operand slots ("XX " each,
