// Headless runner for ch19 exercise 03 / challenge: raw 6502 programs on
// flat RAM with nestest-style logging.
//
//   ch19_03_trace_log_runner --rom BIN [--data ADDR=HEX]... [--cycles N]
//                            [--trace FILE]      (canonical key=value lines)
//                            [--trace-log FILE]  (nestest-style columns)
//
...
int main(int argc, char** argv) {
    ...
    int rc = 0;
    if (log_path != nullptr) {
        // nestest-style columns: capture the TraceRow BEFORE stepping.
        ...
        // canonical key=value trace (compare_trace.py compatible).
        std::ofstream out(trace_path ? trace_path : "/dev/null",
                          std::ios::binary);
        while (!cpu.halted && max_instructions > 0) {
            --max_instructions;
            ...
            std::snprintf(buf, sizeof buf,
                          "pc=%04x op=%02x a=%02x x=%02x y=%02x p=%02x "
                          "sp=%02x cyc=%llu",
