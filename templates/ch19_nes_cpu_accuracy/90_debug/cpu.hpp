// nes6502.hpp — 6502 core, debugging exercise (two seeded bugs).
//
// Someone refactored the accuracy layer and broke two behaviors.
...
inline void op_inx(Cpu& c, uint16_t) { ++c.cycles; ++c.x; c.set_zn(c.x); }
inline void op_iny(Cpu& c, uint16_t) { ++c.cycles; ++c.y; c.set_zn(c.y); }
inline void op_dex(Cpu& c, uint16_t) { ++c.cycles; --c.x; c.set_zn(c.x); }
inline void op_dey(Cpu& c, uint16_t) { ++c.cycles; --c.y; c.set_zn(c.y); }
//@LABS-STUB
// Read-modify-write helper: the real 6502 writes the OLD value back before
// the new one (a dummy write other devices can observe). Billing each
// access gives the official 5-cycle zp / 6-cycle abs RMW totals.
// TODO(1): BUG — this version skips the write-back of the OLD value.
// Sequence must be read, write(old), compute, write(new): the extra bus
// write IS the official 5th cycle of INC $zp, and devices snooping the
// bus never see the original value.
using RmwFn = uint8_t (*)(Cpu&, uint8_t);
inline uint8_t rmw(Cpu& c, uint16_t ad, RmwFn f) {
    const uint8_t old = c.read(ad);
    const uint8_t updated = f(c, old);
...
inline int step(Cpu& c) {
    if (c.halted || c.bus == nullptr) return 0;
    const uint64_t t0 = c.cycles;
    // Poll point: between instructions.
    if (c.nmi_line) {          // TODO(2): BUG — level-sensitive NMI: holding
        service_nmi(c);        // the line asserted re-services it every step
        return int(c.cycles - t0);
    }
    if (c.irq_line && !(c.p & FI)) {
        service_irq(c);
