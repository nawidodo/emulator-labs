...
//
//   ch26_pipeline_runner --rom prog.bin --headless --cycles N --trace t.log
//                         [--dump d.txt]
//
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--rom") rom_path = next();
            ...
            std::printf(
                "usage: ch26_pipeline_runner --rom PATH [--headless]"
                " [--cycles N] [--frames N] [--trace FILE] [--dump FILE]\n");
            return 0;
        }
    }
    if (rom_path.empty()) {
        std::fprintf(stderr, "ch26_pipeline_runner: --rom is required\n");
        return 2;
    }

    thumb::ThumbCpu cpu;
    if (FILE* f = std::fopen(rom_path.c_str(), "rb")) {
        std::fread(cpu.mem, 1, thumb::ThumbCpu::kMemSize, f);
        std::fclose(f);
    } else {
        std::fprintf(stderr, "ch26_pipeline_runner: cannot open %s\n",
                     rom_path.c_str());
        return 2;
    }
