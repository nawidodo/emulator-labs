...
//
//   ch26_switch_runner --rom prog.bin --headless --cycles N --trace t.log
//                         [--dump d.txt]
//
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--rom") rom_path = next();
            ...
            std::printf(
                "usage: ch26_switch_runner --rom PATH [--headless]"
                " [--cycles N] [--frames N] [--trace FILE] [--dump FILE]\n");
            return 0;
        }
    }
    if (rom_path.empty()) {
        std::fprintf(stderr, "ch26_switch_runner: --rom is required\n");
        return 2;
    }

    dual::SwitchCpu cpu;
    if (FILE* f = std::fopen(rom_path.c_str(), "rb")) {
        std::fread(cpu.mem, 1, dual::SwitchCpu::kMemSize, f);
        std::fclose(f);
    } else {
        std::fprintf(stderr, "ch26_switch_runner: cannot open %s\n",
                     rom_path.c_str());
        return 2;
    }
    ...
    for (long long i = 0; i < cycles; ++i) {
        ...
        const bool in_thumb = cpu.t;
        const unsigned op = in_thumb ? cpu.read16(pc) : cpu.read32(pc);
