...
//   LDX #imm, STA long,X, JMP abs, JML long.
...
//@LABS-STUB
// TODO(1): implement multi-byte operand fetch (little-endian: low byte
// at the lower address, fetched first). Each fetch8 already advances PC.
// Stubs return constants so tests run RED.
inline uint16_t fetch16(Cpu&, Mem&) {
    return 0;
}

inline uint32_t fetch24(Cpu&, Mem&) {
    return 0;
}
//@LABS-END
...
// Width-explicit load core. An 8-bit load replaces only the LOW byte
// of A; the hidden high byte B is preserved (W65C816 behavior).
// Returns 1 when the caller must charge the page-cross cycle.
inline int lda_ea(Cpu& c, Mem& m, uint8_t bank, uint16_t addr, bool extra) {
    const uint8_t lo = m.read(bank, addr);
    if (a_is_8bit(c)) {
        c.a = uint16_t((c.a & 0xFF00u) | lo);
    } else {
        c.a = uint16_t(lo) |
              uint16_t(m.read(bank, uint16_t(addr + 1))) << 8;
    }
    set_zn_a(c, c.a);
    return extra ? 1 : 0;
}
...
//@LABS-STUB
// TODO(2): implement width-explicit load/store cores. lda_ea reads the
// accumulator width worth of bytes starting at (bank,addr), masks into
// A, updates Z/N for that width, returns 1 if the caller should charge
// the page-cross cycle (`extra`). sta_ea writes A's current width worth
// of bytes. An 8-bit access touches exactly one byte.
inline int lda_ea(Cpu& c, Mem& m, uint8_t bank, uint16_t addr, bool extra) {
    (void)c;
    (void)m;
    (void)bank;
...
//@LABS-STUB
// TODO(3): implement the instruction dispatcher. Fetch the opcode, then
// switch on it. Every handler must respect CURRENT widths (immediate
// length, how many bytes LDA/STA touch) and return the instruction's
// cycle count. BRK (opcode 0x00) and unknown opcodes halt: return -1.
// Supported set is listed in the file comment above.
inline int step(Cpu& c, Mem& m) {
    (void)c;
    (void)m;
    return -1;
...
//@LABS-STUB
// TODO(4): implement the trace line and disassembler rendering.
// trace_line grammar: "pc=%04X op=%02X k=%02X db=%02X a=%04X x=%04X
// y=%04X p=%02X sp=%04X cyc=%llu" (lowercase keys, cyc last).
// disassemble renders "<K>/<pc>:" then hex bytes of the instruction
// (use insn_len for the width-aware length) then two spaces and the
// mnemonic from mnemonic(). Stubs emit placeholders.
inline std::string trace_line(const Cpu& c, uint16_t pc_at_start,
                              uint8_t op) {
    (void)c;
    (void)pc_at_start;
