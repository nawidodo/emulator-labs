...
//@LABS-STUB
    std::string disasm(uint8_t addr) const override {
        // TODO(1): decode the instruction AT `addr` (peek, no execution)
        // into text — NOP/OUT/HALT bare; LDA/STA/ADD/SUB/JMP/JZ take the
        // operand byte at addr+1 as "$xx" (uppercase hex). Unknown ops:
        // "?? $xx". Pure function of memory.
        (void)addr;
        return "";  // wrong on purpose: empty disassembly
    }
//@LABS-END
...
//@LABS-STUB
    StepInfo step() override {
        // TODO(2): snapshot mem[256], run ONE instruction via
        // cpu_.step(), then diff to fill StepInfo{pc_before, opcode,
        // cycles, write addresses}. Bump per-address watch bookkeeping
        // so watchpoint hit counts stay accurate. Cache the result in
        // last_step_ before returning.
        return {};  // wrong on purpose: never steps
    }
//@LABS-END
...
//@LABS-SOLUTION
    ...
    // Called AFTER each step: exact address compare — no masking,
    // hardware does not round PCs. Counts hits; returns the fired
    // step's info or nullptr.
    const StepInfo* check_breakpoints() {
        for (auto& bp : breakpoints_) {
            if (bp.enabled && cpu_.pc == bp.addr) {
                ++bp.hits;
                return &last_step_;
            }
        }
...
//@LABS-STUB
    void add_breakpoint(uint8_t) override {}
    void add_watchpoint(uint8_t) override {}
    const StepInfo* check_breakpoints() {
        // TODO(3): register breakpoints/watchpoints; implement an EXACT
        // pc compare against enabled entries, counting hits, returning
        // &last_step_ when one fires (nullptr otherwise).
        return nullptr;
    }
//@LABS-END
...
//@LABS-STUB
    std::string regs_json() const override {
        // TODO(4): emit exactly
        // {"a":A,"x":X,"y":Y,"pc":PC,"z":bool,"c":bool,"cycles":N}
        // decimal values, lowercase JSON booleans.
        (void)this;
        return "{}";  // wrong on purpose: empty payload
    }
//@LABS-END
