...
//@LABS-STUB
// TODO(1): give every data-carrying IR insn its semantics against `m`,
// mirroring rx8::execute exactly: r0 writes discarded (but fault effects
// kept!), Alu picks reg or simm operand via use_imm, Store recomputes
// base+simm and — crucially — marks dirty_ when the address lies below
// code_end so self-modifying code flushes stale translations later.
inline void IrEngine::exec_insn(const IrInsn& in) {
    (void)in;
    // wrong on purpose: executes nothing
}
//@LABS-END
...
//@LABS-STUB
// TODO(2): the block-level dispatch loop. While under budget and running:
// fetch the block at m.pc via block_for(), execute its insns counting each
// toward ops_executed (data insns via exec_insn), resolve terminators here
// (Br by br_kind against b.fallthrough/b.taken-in-target, Halt sets
// halted), and AFTER the block finishes call drop_translations()+analyze()
// when dirty_ was set — never mid-block. Fall off the end only defensively.
inline uint64_t IrEngine::run(uint64_t max_ops) {
    (void)max_ops;
    return 0;  // wrong on purpose: the pipeline never runs
}
//@LABS-END
