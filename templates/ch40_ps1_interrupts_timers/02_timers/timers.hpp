#pragma once
//
// ch40 / 02_timers — PS1 root counters 0..2
...
public:
    ...
    //@LABS-STUB
    void write_mode(int /*n*/, uint16_t v) {
        // TODO(1): store the mode word, force the counter to zero and prime
        // the internal phase/flag state exactly like the hardware does.
        regs[0].mode = v;
    }
    //@LABS-END
    ...
    //@LABS-STUB
    uint16_t read_mode(int /*n*/) {
        // TODO(2): OR in the request/reached flags, then clear the two
        // sticky "reached" flags because they were just read.
        return regs[0].mode;
    }
    //@LABS-END
    ...
    //@LABS-STUB
    bool irq_line(int /*n*/) const {
        // TODO(3): expose whether the timer's IRQ output line is asserted.
        return false;
    }
    //@LABS-END
    ...
    //@LABS-STUB
    void tick(const TimerSignals& /*s*/, IrqSink /*sink*/, void* /*user*/) {
        // TODO(4): advance every configured root counter by one cycle,
        // honoring its clock source and sync mode, then raise qualifying
        // interrupts through the sink.
    }
    //@LABS-END
...
private:
    ...
    //@LABS-STUB
    void apply_sync(int /*n*/, bool /*hb*/, bool /*vb*/) {
        // TODO(5): apply per-timer sync-mode behavior.
    }
    bool counts_this_cycle(int /*n*/, const TimerSignals& /*s*/) { return false; }
    void step_counter(int /*n*/) {}
    void deliver_irq(int /*n*/, IrqSink /*sink*/, void* /*user*/) {}
    //@LABS-END
