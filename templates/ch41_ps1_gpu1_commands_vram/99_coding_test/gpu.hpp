#pragma once
// CODING TEST starter — complete GP0/GP1 pipeline EXCEPT the triangle
// rasterization core (signed-area test, fill-convention rule, flat and
// Gouraud walkers, channel resolve), which is left as five LABS tasks.
// Everything else — FIFO, transfers, FILL, rectangles, packet decode,
// clipping and masks — is provided so your result can be hashed against
// the hidden golden directly.
...
//@LABS-STUB
// TODO(1): return the cross product (b-a) x (c-a) as 64-bit.
inline int64_t signed_area2(const RasterVert&, const RasterVert&,
                            const RasterVert&) {
...
//@LABS-STUB
// TODO(2): implement the rule above.
inline constexpr bool is_top_left(int64_t, int64_t) {
...
//@LABS-STUB
// TODO(3): cull non-positive area, build the three lattice edge functions
// (see detail::make_edge), walk the bounding box with incremental steps and
// emit centres passing the top-left-inclusive inside test via write_pixel().
inline void draw_triangle_flat(Vram&, const DrawConfig&, uint32_t,
                               const RasterVert&, const RasterVert&,
                               const RasterVert&) {
    // wrong on purpose
...
//@LABS-STUB
// TODO(4): implement the Q12 barycentric walk documented above (same edge
// machinery as draw_triangle_flat, plus per-pixel channel resolution).
inline void draw_triangle_gouraud(Vram&, const DrawConfig&, const RasterVert&,
                                  const RasterVert&, const RasterVert&) {
    // wrong on purpose
}
...
//@LABS-STUB
// TODO(5): weighted sum of the three endpoint channels in Q12 with
// round-half-up (+2048 before >>12), clamped to 0..255.
inline uint32_t shade_channel(int64_t, int64_t, int64_t, uint32_t, uint32_t,
                              uint32_t) {
    return 0;  // wrong on purpose
}
//@LABS-END
